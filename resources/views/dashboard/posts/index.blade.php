...
<div x-data="{ isOpen: false, postId: '' }">
        ...
        <tr class="even:bg-gray-200">
          ...
          <td class="border border-gray-400! py-2 px-4">
            <div class="flex gap-2 justify-center">
              ...
              <a href="/dashboard/posts/{{ $post->slug }}/edit" class="bg-[#F6DC43] px-2 py-1 rounded-md flex items-center transition hover:bg-yellow-400">
                <iconify-icon icon="material-symbols:edit-document-outline-rounded" width="24" height="24"></iconify-icon>
              </a>
              <button type="button" @click="isOpen = true; postId = '{{ $post->slug }}'" class="!bg-[#E52020] px-2 py-1 rounded-md flex items-center transition hover:!bg-red-500">
      ...
      <div class="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
        ...
        <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
            <form :action="'/dashboard/posts/' + postId" method="POST">
              @method('delete')
              @csrf
